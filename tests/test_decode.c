...
void test_decoder_real_payload(void) {
  ...
  const char expected_message_dict[] =
      ...
      "    \"Message\": \"The value for the property MemorySize is not "
      "valid.\",\n"
      "    \"MessageArgs\": [\n"
      "        \"MemorySize\",\n"
      "        \"4096\"\n"
      "    ],\n"
      "    \"Severity\": \"Warning\",\n"
      "    \"Resolution\": \"Correct the property value and retry the "
      "operation.\",\n"
      "    \"RelatedProperties\": [\n"
  ...
  uint8_t *dict_memory_buf =
      ReadFile("dummy_dictionaries/Memory_v1.bin", &dict_memory_sz);
  uint8_t *dict_message_buf =
      ReadFile("dummy_dictionaries/Message_v1.bin", &dict_message_sz);

  uint8_t *bej_memory_buf =
      ReadFile("dummy_data/memory_bej.bin", &bej_memory_sz);
  uint8_t *bej_message_buf =
      ReadFile("dummy_data/message_bej.bin", &bej_message_sz);
  ...
  OutputStreamInit(&json_memory_output);
  OutputStreamInit(&json_message_output);

  bool success_memory =
      BejDecode(&json_memory_output, &bej_memory, &dict_memory);
  bool success_message =
      BejDecode(&json_message_output, &bej_message, &dict_message);

  TEST_ASSERT_TRUE(success_memory);
  TEST_ASSERT_TRUE(success_message);

  TEST_ASSERT_EQUAL_STRING(expected_memory_dict,
                           (char *)json_memory_output.data);
  TEST_ASSERT_EQUAL_STRING(expected_message_dict,
                           (char *)json_message_output.data);
