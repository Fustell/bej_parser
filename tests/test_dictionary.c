...
void test_load_dictionary_subset_into_buffer_empty(void) {
  ...
  bool ok = LoadDictionarySubsetIntoBuffer(buf, sizeof(buf), 0, 0, entries,
                                               &count);
...
void test_load_dictionary_subset_into_buffer_single_entry(void) {
  uint8_t buf[] = {0x10, 0x01, 0x00, 0x08, 0x00, 0x00, 0x00, 0x04,
                   0x0A, 0x00, 'N',  'a',  'm',  'e',  '\0'};

  DictionaryEntry entries[1];
  size_t count;

  bool ok = LoadDictionarySubsetIntoBuffer(buf, sizeof(buf), 0, 1, entries,
                                               &count);
